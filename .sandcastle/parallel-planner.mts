...
import { runImplementation } from "./lib/sandcastle-runs.mts";
import { planWave, runLimited, type PlannedIssue } from "./lib/scheduler.mts";

const options = parseArgs(process.argv.slice(2), "parallel");
const runId = createRunId("parallel", options.runId);
const summary = createSummary("parallel", runId);
...
try {
  ...
  if (options.schedule === "all") {
    ...
    for (let iteration = 1; iteration <= options.maxIterations! && remaining.length > 0; iteration += 1) {
      ...
      for (const status of statuses.filter((entry) => entry.blocked)) {
        if (!summary.issues.some((entry) => entry.number === status.issue.number)) {
          summary.issues.push({
      ...
      const settled = await runLimited(planned, options.maxParallel, async (plannedIssueInfo) => {
        ...
        return runImplementation(issue, plannedIssueInfo.branch, integrationBranch, options, summary);
      });
      ...
      for (const [index, outcome] of settled.entries()) {
        ...
        if (outcome.status === "rejected") {
          ...
          const commits = outcome.value.commits.map((commit) => commit.sha);
          summary.issues.push({
            number: issue.number,
            title: issue.title,
            branch: selected.branch,
            status: commits.length > 0 ? "completed" : "skipped",
            commits,
            log: logPath(summary, `issue-${issue.number}`),
          });
          if (commits.length > 0) {
    ...
    console.log(`Sandcastle scheduled parallel run complete. Summary: ${summary.runDir}/summary.json`);
  ...
  if (options.dryRun || options.planOnly) {
    ...
    writeSummary(summary);
    console.log(`Sandcastle parallel planning complete. Summary: ${summary.runDir}/summary.json`);
    process.exit(0);
  }
  ...
  if (planned.length === 0) {
    ...
    writeSummary(summary);
    console.log(`No safe parallel issues found. Summary: ${summary.runDir}/summary.json`);
    process.exit(0);
  }

  const settled = await runLimited(planned, options.maxParallel, async (plannedIssueInfo) => {
    const issue = plannedIssue(plannedIssueInfo, unblocked);
    return runImplementation(issue, plannedIssueInfo.branch, summary.baseBranch!, options, summary);
  });
  ...
  writeSummary(summary);
  console.log(`Sandcastle parallel run complete. Summary: ${summary.runDir}/summary.json`);
