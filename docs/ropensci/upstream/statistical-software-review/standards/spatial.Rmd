...
## Spatial Software {#standards-spatial}
...
- [**SP1.0**]{#SP1_0} *Spatial software should explicitly indicate its domain
...
- [**SP1.1**]{#SP1_1} *Spatial software should explicitly indicate its
  ...
  are any other restrictions on dimensionality.*
...
Software](https://stats-devguide.ropensci.org/standards.html#standards-time-series),
...
- [**SP2.0**]{#SP2_0} *Spatial software should only accept input data of one or
  more classes explicitly developed to represent such data.*
    - [**SP2.0a**]{#SP2_0a} *Where new classes are implemented, conversion to
      other common classes for spatial data in R should be documented.*
    - [**SP2.0b**]{#SP2_0b} *Class systems should ensure that functions error
...
- [**SP2.1**]{#SP2_1} *Spatial Software should not use the [`sp`
  package](https://cran.r-project.org/package=sp), rather should use
  [`sf`](https://cran.r-project.org/package=sf).*

More generally,

- [**SP2.2**]{#SP2_2} *Geographical Spatial Software should ensure maximal
  compatibility with established packages and workflows, minimally through:*
    - [**SP2.2a**]{#SP2_2a} *Clear and extensive documentation demonstrating
      how routines from that software may be embedded within, or otherwise
      adapted to, workflows which rely on these established packages; and*
    - [**SP2.2b**]{#SP2_2b} *Tests which clearly demonstrate that routines from
      that software may be successfully translated into forms and workflows
      which rely on these established packages.*

This standard is further refined in a number of subsequent standards concerning
documentation and testing.

- [**SP2.3**]{#SP2_3} *Software which accepts spatial input data in any
  standard format established in other R packages (such as any of the formats
  able to be read by [`GDAL`](https://gdal.org), and therefore by the [`sf`
...
- [**SP2.4**]{#SP2_4} *Geographical Spatial Software should be compliant with
  version 6 or larger of* [`PROJ`](https://proj.org/), *and with* `WKT2`
  *representations. The primary implication, described in detail in the
  articles linked to above, is that:*
    - [**SP2.4a**]{#SP2_4a} *Software should not permit coordinate reference
...
- [**SP2.5**]{#SP2_5} *Class systems for input data must contain meta data on
  associated coordinate reference systems.*
    - [**SP2.5a**]{#SP2_5a} *Software which implements new classes to input
      ...
      classes such as those listed above.*
- [**SP2.6**]{#SP2_6} *Spatial Software should explicitly document the types
  and classes of input data able to be passed to each function.*
- [**SP2.7**]{#SP2_7} *Spatial Software should implement validation routines to
...
- [**SP2.8**]{#SP2_8} *Spatial Software should implement a single
  ...
  functions.*
- [**SP2.9**]{#SP2_9} *The pre-processing function described above should
  maintain those metadata attributes of input data which are relevant or
  important to core algorithms or return values.*
...
package](https://github.com/ropensci-review-tools/srr).

- [**SP3.0**]{#SP3_0} *Spatial software which considers spatial neighbours
  should enable user control over neighbourhood forms and sizes. In
  particular:*
    - [**SP3.0a**]{#SP3_0a} *Neighbours (able to be expressed) on regular grids
      should be able to be considered in both rectangular only, or rectangular
      and diagonal (respectively "rook" and "queen" by analogy to chess).*
    - [**SP3.0b**]{#SP3_0b} *Neighbourhoods in irregular spaces should be
      ...
      neighbours, or otherwise equivalent user-controlled value.*
- [**SP3.1**]{#SP3_1} *Spatial software which considers spatial neighbours
  ...
  on a uniform-weight rectangular cut-off.*
- [**SP3.2**]{#SP3_2} *Spatial software which relies on sampling from input
  data (even if only of spatial coordinates) should enable sampling procedures
  to be based on local spatial densities of those input data.*
...
- [**SP3.3**]{#SP3_3} *Spatial regression software should explicitly quantify
...
- [**SP3.4**]{#SP3_4} *Where possible, spatial clustering software should avoid
...
- [**SP3.5**]{#SP3_5} *Spatial machine learning software should ensure that
...
corresponding [Machine Learning Standards](#ml-standards), just
above *Input Data Specification*.

- [**SP3.6**]{#SP3_6} *Spatial machine learning software should document (and,
  where possible, test) the potential effects of different sampling procedures*

A simple example might be to provide examples or extended documentation which
...
no comparable *General Standard* for [*Machine Learning
Software*](#ml-standards), procedures for sampling spatial data may have
...
A more concrete example may be to demonstrate a particular technique for
...
explicitly documented (and possibly tested as well).
...
- [**SP4.0**]{#SP4_0} *Return values should either:*
    - [**SP4.0a**]{#SP4_0a} *Be in same class as input data, or*
    - [**SP4.0b**]{#SP4_0b} *Be in a unique, preferably class-defined, format.*
- [**SP4.1**]{#SP4_1} *Any aspects of input data which are included in output
  data (either directly, or in some transformed form) and which contain units
  should ensure those same units are maintained in return values.*
- [**SP4.2**]{#SP4_2} *The type and class of all return values should be
...
- [**SP5.0**]{#SP5_0} *Implement default `plot` methods for any implemented
  class system.*
- [**SP5.1**]{#SP5_1} *Implement appropriate placement of variables along x-
  and y-axes.*
- [**SP5.2**]{#SP5_2} *Ensure that axis labels include appropriate units.*
...
- [**SP5.3**]{#SP5_3} *Offer an ability to generate interactive (generally
...
- [**SP6.0**]{#SP6_0} *Software which implements routines for transforming
...
- [**SP6.1**]{#SP6_1} *All functions which can be applied to both Cartesian and
  curvilinear data should be tested through application to both.*
    - [**SP6.1a**]{#SP6_1a} *Functions which may yield inaccurate results when
      ...
      accurate.*
    - [**SP6.1b**]{#SP6_1b} *Functions which yield accurate results regardless
...
- [**SP6.2**]{#SP6_2} *Geographical Software should include tests with extreme
...
- [**SP6.3**]{#SP6_3} *Spatial Software which considers spatial neighbours
  ...
  neighbours.*
- [**SP6.4**]{#SP6_4} *Spatial Software which considers spatial neighbours
  should explicitly test effects of different schemes to weight neighbours by
  spatial proximity.*

**Unsupervised Learning Software**

- [**SP6.5**]{#SP6_5} *Spatial Unsupervised Learning Software which uses
  clustering algorithms should implement tests which explicitly compare results
  with equivalent results obtained with a non-spatial clustering algorithm.*

**Machine Learning Software**

- [**SP6.6**]{#SP6_6} *Spatial Machine Learning Software should implement tests
