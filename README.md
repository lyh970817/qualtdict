...
`qualtRics::fetch_survey()` with the response-column settings needed to
match downloaded data back to `response_column_id`, then renames Export
Variables to `variable_name` and attaches labels from the Variable
Dictionary.
...
citation for the installed version is returned by `citation()`.
...
- The [contributing guide](.github/CONTRIBUTING.md) describes how to
  propose a change.
