...
`.claude/worktrees/` at the repository root. Both directories are
ignored by Git and keep parallel checkouts out of the main source tree.
Once a worktree’s branch is merged, remove the worktree and delete the
merged branch; do not leave them accumulating.
