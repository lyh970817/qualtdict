...
import { implementationPrompt } from "./lib/prompts.mts";
...
const options = parseArgs(process.argv.slice(2), "single");
const runId = createRunId("single", options.runId);
const summary = createSummary("single", runId);
...
try {
  ...
  if (options.dryRun) {
    ...
    summary.notes.push("Dry run printed implementation prompt without running Codex.");
    writeSummary(summary);
    console.log(implementationPrompt(issue));
    process.exit(0);
  }

  const result = await runImplementation(issue, branch, summary.baseBranch, options, summary);
  summary.issues[0]!.status = result.commits.length > 0 ? "completed" : "skipped";
  summary.issues[0]!.commits = result.commits.map((commit) => commit.sha);
  summary.status = "completed";
  writeSummary(summary);

  console.log(`Sandcastle single run complete. Summary: ${summary.runDir}/summary.json`);
} catch (error) {
