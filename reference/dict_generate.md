...
downloaded Response Column ID, `qid` as the bare Qualtrics question
...
`row_source = "scoring"`. Text-analysis Sidecars discovered from
Response Column Map Classification use `row_source = "text_analysis"`
and inherit parent `qid`, `question_name`, and `block` when a clear
...
Survey Flow adjacency assignment. That assignment is based on Survey
Flow returned by `fetch_description()`, where Embedded Data nodes carry
field payloads; the type-only Survey Flow returned by `metadata()` is
not used for Embedded Data Field locations. Scoring Variables remain
